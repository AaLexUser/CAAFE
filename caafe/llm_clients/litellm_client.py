...
from caafe.configs.app_config import AppConfig
...
class LiteLLMClient:
    def __init__(self, config: AppConfig):
        ...
        self.completion_params = {
            "model": f"{self.config.llm.model_name}",
        ...
        if self.config.llm.caching.enabled:
            litellm.cache = Cache(
                type=LiteLLMCacheType.DISK,
                disk_cache_dir=self.config.llm.caching.dir_path,
            )
