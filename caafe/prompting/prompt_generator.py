...
from .utils import extract_code, extract_first_tool_call
...
class PromptGenerator(ABC):
    ...
    def create_parser(self):
        # Parser that first attempts to interpret the response as a tool call block.
        # If a tool call is found and contains a 'code' argument, return that; else fallback to code fence extraction.
        def _parser(response: str):
            tool = extract_first_tool_call(response)
            if tool:
                args = tool.get("args", {})
                # common argument names that may contain code
...
class FeatureGenerationPromptGenerator(PromptGenerator):
    def __init__(self, description: str, train_df: pd.DataFrame, target_column: str, how_many: int = 1):
        super().__init__()
